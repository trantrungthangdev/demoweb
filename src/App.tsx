...
export default function CinemaVault() {
  ...
  const handleMoveToTrash = async (item: any) => {
    ...
    try {
      ...
      if (fileBlob) {
        ...
        await supabase.from(item.origin_table).update({ name: `${item.name}_trash`, url: publicUrl, is_deleted: true }).eq('id', item.id);
      }
    } catch (e: any) { alert("Lỗi di chuyển tệp: Kiểm tra Bucket 'trash'"); }
  ...
  const handleRestore = async (item: any) => {
    ...
    try {
      ...
      if (fileBlob) {
        ...
        await supabase.from(item.origin_table).update({ name: item.name.replace('_trash', ''), url: publicUrl, is_deleted: false }).eq('id', item.id);
      }
  ...
  const handlePermanentDelete = async (id: string, itemUrl: string, table: string) => {
    if (confirm("Xoá vĩnh viễn tệp này khỏi bộ nhớ?")) {
      setIsLoading(true);
      try {
  ...
  const handleClearTrash = async () => {
    ...
    if (!confirm(`Bạn có chắc muốn xoá vĩnh viễn TẤT CẢ ${media.length} tệp trong thùng rác?`)) return;
    setIsLoading(true);
    try {
      for (const item of media) {
        const fileName = item.url.split('/').pop();
        await supabase.storage.from('trash').remove([fileName]);
        await supabase.from(item.origin_table).delete().eq('id', item.id);
      }
      await fetchMedia();
  ...
  if (!isLogged) return (
    <div className="h-screen bg-black flex items-center justify-center p-6 font-sans text-white text-center">
      <div className="w-full max-w-sm bg-zinc-900 border border-white/5 p-10 rounded-[3rem] shadow-2xl">
        ...
        <input type="password" autoFocus placeholder="••••" className="w-full bg-black border border-zinc-800 p-4 rounded-2xl text-center text-2xl tracking-[0.5em] outline-none focus:border-red-600" onKeyDown={(e) => { if (e.key === 'Enter' && (e.target as any).value === ACCESS_KEY) { setIsLogged(true); localStorage.setItem('vault_logged_in', 'true'); }}} />
      </div>
    </div>
  );
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1 md:gap-2">
              ...
              {media.map((m) => (
                <div key={m.id} onMouseDown={() => touchTimer.current = setTimeout(() => setLongPressedId(m.id), 600)} onMouseUp={() => clearTimeout(touchTimer.current)} onTouchStart={() => touchTimer.current = setTimeout(() => setLongPressedId(m.id), 600)} onTouchEnd={() => clearTimeout(touchTimer.current)} onClick={(e) => { e.stopPropagation(); setViewing(m); }} className="relative aspect-square group cursor-pointer overflow-hidden rounded-lg bg-zinc-900">
                  ...
                  {isVideoFile(m.url) && <div className="absolute bottom-2 right-2 bg-black/60 px-2 py-1 rounded text-[8px] font-black uppercase"><Play size={8} fill="currentColor" className="inline mr-1"/> Video</div>}
                </div>
              ))}
            </div>
          )}
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in">
          <div className="w-full max-w-xs bg-zinc-900 border border-white/10 p-8 rounded-[2.5rem] text-center shadow-2xl">
            ...
            <p className="text-zinc-500 text-xs mb-8 uppercase font-bold tracking-tight">Xác nhận để thoát khỏi kho lưu trữ.</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={handleConfirmLogout} className="py-4 bg-red-600 rounded-2xl text-[10px] font-black uppercase text-white">Đăng xuất</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-black flex flex-col animate-in fade-in" onMouseMove={handleUserActivity} onClick={handleUserActivity}>
          <header className={`absolute top-0 inset-x-0 z-20 flex justify-between items-start p-4 md:p-8 bg-gradient-to-b from-black/90 to-transparent transition-opacity duration-500 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="flex gap-2 shrink-0">
              <button onClick={(e) => {e.stopPropagation(); handleMoveToTrash(viewing); setViewing(null);}} className="w-10 h-10 md:w-14 md:h-14 bg-red-600/20 text-red-500 rounded-xl flex items-center justify-center transition-all hover:bg-red-600 hover:text-white"><Trash2 size={24}/></button>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-4 md:gap-10">
                      ...
                      <div className="hidden md:flex items-center gap-3">
                        <button onClick={(e) => {e.stopPropagation(); setIsMuted(!isMuted);}} className="text-zinc-400 hover:text-white transition-colors">{isMuted || volume === 0 ? <VolumeX size={20} /> : <Volume2 size={20} />}</button>
