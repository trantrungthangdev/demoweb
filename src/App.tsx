...
export default function CinemaVault() {
  ...
  const [isLogged, setIsLogged] = useState<boolean>(() => {
    return localStorage.getItem('vault_logged_in') === 'true';
  });
  ...
  // --- GIỮ NGUYÊN TOÀN BỘ PLAYER STATES CŨ (CẤM XOÁ) ---
  ...
  // --- HÀM DỌN DẸP TÊN FILE (FIX LỖI INVALID KEY) ---
  const sanitizeFileName = (name: string) => {
    return name
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '') // Khử dấu VN
      .replace(/[^\x00-\x7F]/g, '') // Xóa ký tự lạ (Nhật, Trung...)
  ...
  const fetchMedia = useCallback(async () => {
    ...
    try {
      for (const table of tables) {
        ...
        if (!error && data) {
          allData = [...allData, ...data.map(item => ({ ...item, origin_table: table }))];
        }
      }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        alert("Lỗi tải lên: " + error.message + ". Đảm bảo đã chỉnh Policy cho bucket " + bucketName);
      }
    }
  ...
  // --- LOGIC XOÁ & KHÔI PHỤC (DI CHUYỂN GIỮA BUCKET) ---
  const handleMoveToTrash = async (item: any) => {
    ...
    try {
      ...
      alert("Lỗi xóa: Hãy kiểm tra Policy của bucket 'trash'");
    }
  ...
  const handlePermanentDelete = async (id: string, itemUrl: string, table: string) => {
    if (confirm("Xoá vĩnh viễn tệp khỏi hệ thống?")) {
      setIsLoading(true);
      try {
  ...
  // --- TOÀN BỘ LOGIC PLAYER GỐC (CẤM XOÁ) ---
  const togglePlay = () => {
    if (videoRef.current) {
  ...
  if (!isLogged) return (
    <div className="h-screen bg-black flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm bg-zinc-900 border border-white/5 p-10 rounded-[3rem] text-center shadow-2xl">
        ...
        <h1 className="text-white text-2xl font-black italic uppercase mb-8">VAULT</h1>
        <input 
          ...
          onKeyDown={(e) => { if (e.key === 'Enter' && (e.target as any).value === ACCESS_KEY) {
            setIsLogged(true); localStorage.setItem('vault_logged_in', 'true');
          }}}
        />
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#070707] text-zinc-100 flex flex-col md:flex-row font-sans overflow-hidden" onClick={() => setLongPressedId(null)}>
      ...
      {(isLoading || uploading) && (
        <div className="fixed top-0 left-0 right-0 h-1 z-[110] bg-zinc-800">
          <div className="h-full bg-red-600 transition-all duration-300" style={{ width: uploading ? `${uploadProgress}%` : '60%' }}></div>
        </div>
      )}

      <nav className="fixed bottom-0 left-0 w-full h-16 bg-zinc-900/95 backdrop-blur-2xl border-t border-white/5 flex flex-row items-center justify-around z-[60] md:relative md:w-20 md:h-screen md:flex-col md:border-t-0 md:border-r md:bg-black md:px-4 md:py-8 lg:w-64">
        ...
        <button onClick={() => {setIsLogged(false); localStorage.removeItem('vault_logged_in');}} className="p-4 text-zinc-500 hover:text-red-500"><LogOut size={22}/></button>
      </nav>
      ...
      <main className="flex-1 h-screen overflow-y-auto pb-20 md:pb-0">
        <header className="sticky top-0 z-40 bg-black/80 backdrop-blur-md px-6 py-4 flex items-center justify-between border-b border-white/5">
          ...
          <label className={`cursor-pointer ${uploading ? 'bg-zinc-800' : 'bg-white text-black'} px-5 py-2 rounded-xl text-xs font-black uppercase flex items-center gap-2`}>
          ...
          {type === 'library' ? (
            <div className="max-w-xl mx-auto space-y-3">
              <div onClick={() => {setTab('trash'); setType('video');}} className="flex items-center justify-between p-6 bg-zinc-900 rounded-[2rem] border border-white/5 hover:bg-zinc-800 transition-all cursor-pointer">
                <div className="flex items-center gap-4"><div className="w-12 h-12 bg-red-600/10 rounded-2xl flex items-center justify-center text-red-500"><Trash2/></div><div><p className="font-black uppercase italic">Thùng rác</p></div></div><ChevronRight/>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1 md:gap-2">
              {media.length === 0 && !isLoading && <div className="col-span-full py-20 text-center opacity-20 uppercase font-black italic text-4xl">Trống trơn</div>}
              {media.map((m) => (
                <div key={m.id} onMouseDown={() => touchTimer.current = setTimeout(() => setLongPressedId(m.id), 600)} onMouseUp={() => clearTimeout(touchTimer.current)} onTouchStart={() => touchTimer.current = setTimeout(() => setLongPressedId(m.id), 600)} onTouchEnd={() => clearTimeout(touchTimer.current)} onClick={(e) => { e.stopPropagation(); setViewing(m); }} className="relative aspect-square group cursor-pointer overflow-hidden rounded-lg bg-zinc-900">
                  {isVideoFile(m.url) ? <video src={`${m.url}#t=0.5`} className="w-full h-full object-cover pointer-events-none" muted playsInline /> : <img src={m.url} className="w-full h-full object-cover" />}
                  {longPressedId === m.id && (
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center gap-3 z-10 animate-in fade-in">
                      {tab === 'trash' && <button onClick={(e) => {e.stopPropagation(); handleRestore(m)}} className="w-12 h-12 bg-white text-black rounded-full flex items-center justify-center shadow-xl"><RotateCcw size={20}/></button>}
                      <button onClick={(e) => {e.stopPropagation(); tab === 'trash' ? handlePermanentDelete(m.id, m.url, m.origin_table) : handleMoveToTrash(m)}} className="w-12 h-12 bg-red-600 text-white rounded-full flex items-center justify-center shadow-xl"><Trash2 size={20}/></button>
                    </div>
                  )}
                  {isVideoFile(m.url) && <div className="absolute bottom-2 right-2 bg-black/60 px-2 py-1 rounded text-[8px] font-black uppercase tracking-widest"><Play size={8} fill="currentColor" className="inline mr-1"/> Video</div>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-[100] bg-black flex flex-col animate-in fade-in" onMouseMove={handleUserActivity} onClick={handleUserActivity}>
          <header className={`absolute top-0 inset-x-0 z-20 flex justify-between items-start p-4 md:p-8 bg-gradient-to-b from-black/90 to-transparent transition-opacity duration-500 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="flex gap-2 shrink-0">
              <button onClick={(e) => {e.stopPropagation(); handleMoveToTrash(viewing); setViewing(null);}} className="w-10 h-10 md:w-14 md:h-14 bg-red-600/20 text-red-500 rounded-xl flex items-center justify-center"><Trash2 size={20}/></button>
              <a href={viewing.url} download className="w-10 h-10 md:w-14 md:h-14 bg-white/10 hover:bg-white/20 rounded-xl flex items-center justify-center"><Download size={22}/></a>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-4 md:gap-10">
                      <div className="flex items-center gap-2 md:gap-6">
                        <button onClick={(e) => {e.stopPropagation(); skipTime(-10);}} className="text-zinc-400 hover:text-white"><Rewind size={24} /></button>
                        ...
                        <button onClick={(e) => {e.stopPropagation(); skipTime(10);}} className="text-zinc-400 hover:text-white"><FastForward size={24} /></button>
                      </div>
                      <div className="hidden md:flex items-center gap-3">
                        <button onClick={(e) => {e.stopPropagation(); setIsMuted(!isMuted);}} className="text-zinc-400">{isMuted ? <VolumeX size={20} /> : <Volume2 size={20} />}</button>
                        <input type="range" min="0" max={1} step="0.1" value={isMuted ? 0 : volume} onChange={(e) => { const val = parseFloat(e.target.value); setVolume(val); if (videoRef.current) videoRef.current.volume = val; }} className="w-20 transition-all appearance-none bg-zinc-700 h-1 rounded-full accent-white" />
                      </div>
                    </div>
                    <div className="flex items-center gap-4 md:gap-8">
                      <div className="flex bg-zinc-900/80 rounded-lg p-1 border border-white/5">
                        {[1, 1.5, 2].map(speed => (<button key={speed} onClick={(e) => { e.stopPropagation(); if (videoRef.current) videoRef.current.playbackRate = speed; setPlaybackRate(speed); }} className={`px-3 py-1 text-[9px] font-black rounded-md transition-all ${playbackRate === speed ? 'bg-red-600 text-white' : 'text-zinc-500'}`}>{speed}x</button>))}
                      </div>
                      <button onClick={(e) => {e.stopPropagation(); toggleFullScreen();}} className="text-zinc-400 hover:text-white"><Maximize size={22} /></button>
                    </div>
                  </div>
                </div>
              </>
